...
import { getAppointments, getClinicalNotes, getCurrentUserContext, getPatients, scopePatientsToCurrentProvider, scopeRecordsToCurrentProvider } from "@/lib/data/ehr";
...
export default async function NotesPage({
  ...
  const [notes, appointments, patients] = await Promise.all([getClinicalNotes(), getAppointments(), getPatients()]);
            ...
            {scopedNotes.map((note) => (
              <details key={`${note.id}-edit`} className="rounded-2xl border border-white/60 bg-white/80 p-4 shadow-sm">
                ...
                <form action={updateNoteAction} className="mt-4 grid gap-3">
                  ...
                  <input type="hidden" name="providerId" value={note.providerId} />
                  <input type="hidden" name="redirectTo" value="/notes" />
